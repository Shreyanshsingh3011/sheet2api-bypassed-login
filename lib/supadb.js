...
// Read a single row from a RELATIONAL dbridge table (real columns, not the
// {id, data} jsonb shape the collection() adapter assumes). Matches `column`
// against `value` and returns the raw row, or null. Never throws.
export async function getRelationalRow(table, column, value) {
  if (!SUPABASE_URL || !SUPABASE_KEY) return null
  try {
    const url = `${rest(table)}?${encodeURIComponent(column)}=eq.${encodeURIComponent(value)}&select=*&limit=1`
...
// Per-token header override from dbridge_header_mappings (or null).
export function getHeaderMapping(token) {
  return getRelationalRow('dbridge_header_mappings', 'token', token)
}
